    ...
    const handleScroll = () => {
      ...
      if (!currentActiveId && headingElements.length > 0) {
        const firstHeadingTop =
          headingElements[0].getBoundingClientRect().top + scrollY;
    ...
    return items.map((item) => (
      <li key={item.slug}>
        ...
        {item.children.length > 0 && (
          <ul className="mt-2.5 ml-5 flex list-none flex-col gap-y-2.5">
      ...
      <ScrollArea className="h-[calc(100vh-8rem)]">
        <nav>
          <ul className="flex list-none flex-col gap-y-3">
            {renderTocItems(groupedHeadings)}
          </ul>
        </nav>
      </ScrollArea>
    </div>
