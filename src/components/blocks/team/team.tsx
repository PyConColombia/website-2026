...
const TeamMemberSocialLinks = ({ member }: { member: TeamMember }) => (
  <div className='flex items-center gap-3'>
    ...
    {member.x ? (
      <Button size='icon' asChild className={socialButtonClassName}>
        <a href={member.x} target='_blank' rel='noopener noreferrer' aria-label={`${member.name} X`}>
  ...
  return (
    <section id='team' className='py-8 sm:py-16 lg:py-24'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        ...
        >
          <p className='text-primary text-sm font-medium uppercase'>{t('blocks.team.eyebrow')}</p>
          <h2 className='text-2xl font-semibold md:text-3xl lg:text-4xl'>{t('blocks.team.title')}</h2>
          <p className='text-muted-foreground text-xl'>{t('blocks.team.subtitle')}</p>
        </MotionPreset>
        ...
        <div className='mx-auto grid w-full max-w-6xl grid-cols-1 gap-8 md:grid-cols-6 md:gap-12 lg:gap-16 2xl:gap-24'>
          {teamMembers.map((member, index) => (
            <MotionPreset
              ...
              transition={{ duration: 0.5 }}
              className={cn('h-full w-full md:col-span-2', getCenteredRowClassName(index))}
            >
